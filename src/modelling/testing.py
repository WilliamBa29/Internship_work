...
def testing(x1,x2,Y1,Y2,notestpoints,df_,scaler1,scaler,num_inducing,kernel,mean_function,I,d,t1,t2):
        ...
        if i==1:
         ...
         #print(gpr.kernel)
          ...
          #   kernel= gp.kernels.Sum(kernel1,kernel2)    
            ...
            #kernel3=gp.kernels.Linear(input_dim=1,variance=torch.tensor(gpr.kernel.kern1.variance.item()))
           # kernela= gp.kernels.Sum(kernel1,kernel2)
          #  kernel=gp.kernels.Sum(kernela,kernel3) 
         #if I==7:
            #kernel1=gp.kernels.RBF(input_dim=1,variance=torch.tensor(gpr.kernel.kern0.variance.item()),lengthscale=torch.tensor(gpr.kernel.kern0.lengthscale.item()))
            #kernel2=gp.kernels.Periodic(input_dim=1,variance=torch.tensor(gpr.kernel.kern1.variance.item()),lengthscale=torch.tensor(gpr.kernel.kern1.lengthscale.item()))
           # kernel= gp.kernels.Product(kernel1,kernel2)        
         gpr.set_data(torch.tensor(np.concatenate((x1,x2[range(0,(i-1)*notestpoints)]),axis=0).astype(float)), y=torch.transpose(torch.tensor(np.concatenate((Y1,Y2[range(0,(i-1)*notestpoints),:]),axis=0).astype(float)),0,1))
         #gpr = gp.models.SparseGPRegression(X=torch.tensor(np.concatenate((x1,x2[range(0,(i-1)*notestpoints),:]),axis=0).astype(float)), y=torch.transpose(torch.tensor(np.concatenate((Y1,Y2[range(0,(i-1)*notestpoints),:]),axis=0).astype(float)),0,1), kernel=kernel, mean_function=mean_function,Xu=gpr.Xu.data, jitter=1.0e-5)            
        for j in range(1,min(notestpoints+1,len(x2)-(i-1)*notestpoints+1)):
            #print(j)
            if j==1:
             #print(np.shape(x2[(i-1)*notestpoints+j-1,:]))
             c, post_cov = gpr.forward(torch.tensor(x2[(i-1)*notestpoints+j-1,:].reshape(1,-1).astype(float)), full_cov=False)
             post_mean[j-1,i-1]=c.detach().numpy()
             post_cov1[j-1,i-1]=post_cov.detach().numpy()
            else:
                #print(post_mean.numpy())
                X=np.concatenate((Y1,Y2[range(0,(i-1)*notestpoints)]),axis=0)
                #print(np.shape(X))
                #print(np.shape(post_mean[range(0,j-1)]))
                #print(np.shape(X.reshape(-1,1)))
                #print(np.shape(post_mean[range(0,j-1),i-1].reshape(1,-1)))
                X_good=np.concatenate((X.reshape(-1,1),np.transpose(post_mean[range(0,j-1),i-1].reshape(1,-1))),axis=0)
                h=np.concatenate((df_.iloc[range(0,len(x1)+(i-1)*notestpoints),3],(post_mean[range(0,j-1),i-1]>0)))
                x=np.empty(12,dtype='object')
                x[range(0,10)]=x2[(i-1)*notestpoints+j-1,range(0,10)]
                #print(X_good[range(len(x1)+(i-1)*notestpoints+j-30,len(x1)+(i-1)*notestpoints+j-1)].dtype)
                x[10]=np.mean(X_good[range(len(x1)+(i-1)*notestpoints+j-30*48,len(x1)+(i-1)*notestpoints+j-1)])
                 
                x[11]=np.mean(h[range(len(x1)+(i-1)*notestpoints+j-30*48,len(x1)+(i-1)*notestpoints+j-1)])
                c, post_cov = gpr.forward(torch.tensor(x.reshape(1,-1).astype(np.float)), full_cov=False)
                post_mean[j-1,i-1]=c.detach().numpy()  
                post_cov1[j-1,i-1]=post_cov.detach().numpy()
    M=post_mean.flatten()
    M=M[M!=None]
    ...
    rae1=RAE(Y2,Y1,x2.astype(float),x1.astype(float),M,scaler1,scaler,d)
...
def RAE(Y2,Y1,x2,x1,posterior_mean,scaler1,scaler,d):
    ...
    #print(np.array_equal(x1,x2,equal_nan=True))
     ...
     if np.array_equal(x1,x2,equal_nan=True)==False:
      x1[[0,1,9,10,11],:]=scaler.inverse_transform(x1[[0,1,9,10,11],:])
      x2[[0,1,9,10,11],:]=scaler.inverse_transform(x2[[0,1,9,10,11],:])
     else:
      x1[[0,1,9,10,11],:]=scaler.inverse_transform(x1[[0,1,9,10,11],:])
      x2=x1
    ...
    for z in range(0,48):
        interval_mean[z]=np.mean(Y1[x1[:,0].astype(int)==z+1])
    for i in range(0,len(Y2)):
       # print(posterior_mean[i])
        sum2=sum2+abs(Y2[i]-posterior_mean[i])
    for j in range(0, len(Y2)):
       # print(Y2[j])
        sum1=sum1+ abs(Y2[j]-interval_mean[int(x2[j,0]-1)])
    #print(sum1)
    #print(sum)
    #print(interval_mean)
