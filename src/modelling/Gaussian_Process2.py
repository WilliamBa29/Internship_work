...
def GPenergy2(x, x1, data, validationdata, kernel, meanf):
    ...
    gpr = gp.models.GPRegression(X=xj, y=Y, kernel=kernel, mean_function=meanf,
                                 noise=torch.tensor(1.))
    # print(gpr.model())
    # print(gpr.parameters())
    gpr.kernel.lengthscale = pyro.nn.PyroSample(dist.Uniform(0, 200))
    gpr.kernel.variance = pyro.nn.PyroSample(dist.Uniform(0, 20000))
    ...
    for i in range(0, len(x1)):
        ...
        p_value[i] = 1 - scipy.stats.norm.cdf(300, loc=mean1[i].detach().numpy(),
                                              scale=cov[i].detach().numpy())
    import matplotlib.pyplot as plt
    if len(np.shape(x1)) > 1:
        plt.close()
        plt.plot(x1[:, 0], mean1.detach().numpy(), label='Mean (Spot price) of the GP Posterior')
        plt.plot(x1[:, 0], validationdata, label='Spot Price')
        plt.plot(x1[:, 0], lower,
                 label='Lower Bound of Posterior Spot Price 95% Predictive Interval')
        plt.plot(x1[:, 0], upper,
                 label='Upper Bound of Posterior Spot Price 95% Predictive Interval')
        plt.legend()
        plt.show()
    else:
        plt.close()
